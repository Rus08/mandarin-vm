...
int CodeDataInstruction(struct String* pStrings, int StringNum, struct Segment* pCodeSeg, struct Segment* pDataSeg, char* pOutBuf, int StringOffset)
{
	...
	switch(pString->id){
		case VM_DWORD:
		{
			if(pString->repeat == 0){
				for(int i = 0; i < pString->opnum; i++){
					if(DecodeOperand(pString->op[i], 0xffffffff, &lastopintflag, IfSigned(pString->id), StringOffset + StringNum, pString->offset, pCodeSeg, pDataSeg, &op) != 0){
						return -1;
					}
				...
				for(int i = 0; i < pString->repeat; i++){
					if(DecodeOperand(pString->op[0], 0xffffffff, &lastopintflag, IfSigned(pString->id), StringOffset + StringNum, pString->offset, pCodeSeg, pDataSeg, &op) != 0){
						return -1;
					}
					((unsigned int*)pOutBuf)[i] = op;
				}
		...
		case VM_WORD:
		{
			if(pString->repeat == 0){
				for(int i = 0; i < pString->opnum; i++){
					if(DecodeOperand(pString->op[i], 0xffff, &lastopintflag, IfSigned(pString->id), StringOffset + StringNum, pString->offset, pCodeSeg, pDataSeg, &op) != 0){
						return -1;
				 	}
				...
				for(int i = 0; i < pString->repeat; i++){
					if(DecodeOperand(pString->op[0], 0xffff, &lastopintflag, IfSigned(pString->id), StringOffset + StringNum, pString->offset, pCodeSeg, pDataSeg, &op) != 0){
						return -1;
					}
					((unsigned short*)pOutBuf)[i] = op;
				}
		...
		case VM_BYTE:
		{
			if(pString->repeat == 0){
				for(int i = 0; i < pString->opnum; i++){
					if(DecodeOperand(pString->op[i], 0xff, &lastopintflag, IfSigned(pString->id), StringOffset + StringNum, pString->offset, pCodeSeg, pDataSeg, &op) != 0){
						return -1;
					}
				...
				for(int i = 0; i < pString->repeat; i++){
					if(DecodeOperand(pString->op[0], 0xff, &lastopintflag, IfSigned(pString->id), StringOffset + StringNum, pString->offset, pCodeSeg, pDataSeg, &op) != 0){
						return -1;
					}
					((unsigned char*)pOutBuf)[i] = op;
				}
